...
impl BehaviorTree {
    ...
    pub fn tick(&mut self, observation: &Observation) -> (Vec<Action>, BehaviorStatus) {
        ...
        // Temporarily take root out to avoid double-mutable-borrow of self
        let mut root = std::mem::replace(
            &mut self.root,
            BehaviorNode::ActionNode { action: Action::Idle },
        );
        let status = self.tick_node(&mut root, &mut actions, observation);
        self.root = root;
        (actions, status)
    }
...
mod tests {
    ...
    use pod_core::observation::SelfState;
