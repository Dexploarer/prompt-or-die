...
pub mod behavior_nodes;
pub mod fsm_templates;
pub mod toon_encoding;
pub mod hybrid_agent;

pub use llm_agent::{LlmAgent, LlmAgentConfig, DecisionTrace};
pub use llm_provider::{
    LlmProvider, ChatMessage, ChatRole, ChatCompletionRequest, ChatCompletionResponse,
    FinishReason, TokenUsage, LlmError, MockLlmProvider,
};
...
pub use onnx_provider::{
    OnnxError, OnnxActionSelector, StubOnnxPolicyNetwork, softmax,
};
#[cfg(feature = "onnx")]
pub use onnx_provider::OnnxPolicyNetwork;
pub use utility_ai::{UtilityAgent, UtilityBrain, ResponseCurve};
pub use conversation_memory::{
    ConversationMemory, SummarizingMemory, MemoryConfig, estimate_tokens,
};
pub use output_parser::{
    OutputParser, ParseError, JsonOutputParser, PlainTextParser, ChainedParser,
};
pub use prompt_template::{
    PromptTemplate, DefaultPromptTemplate, CompactPromptTemplate,
    PromptBuilder, TemplateSections,
};
pub use token_budget::{
    TokenBudget, TokenAllocator, TokenAllocation, TokenTracker, BudgetConfig,
};
pub use decision_log::{
    DecisionLogger, DecisionFilter, DecisionStats, DecisionExporter,
};
pub use behavior_nodes::{
    Condition, conditional, reactive_patrol, smart_chase, smart_flee,
    smart_guard, random_wander, repeat, timeout, cooldown,
    combat_behavior, sentry_behavior, coward_behavior,
};
pub use fsm_templates::{
    FsmCondition, FsmStateConfig, SmartFsm, SmartTransition,
    combat_fsm, patrol_fsm, guard_fsm,
};
