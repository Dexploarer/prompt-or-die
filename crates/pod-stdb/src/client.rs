...
mod tests {
    ...
    fn test_apply_remote_topology_resolves_world_and_team_metadata() {
        ...
        client.apply_remote_topology(pod_core::RemoteTopologyBundle {
            ...
            tournament_control_plane: pod_core::TournamentControlPlaneSummary {
                ...
                standings: vec![pod_core::TournamentTeamStandingSummary {
                    ...
                    dataset_row_count: 1,
                    world_reward_total: 1.5,
                    applied_score_delta: 4,
            ...
            tournament_orchestration: pod_core::TournamentOrchestrationSummary {
                ...
                worlds: vec![pod_core::WorldTournamentOrchestrationSummary {
                    ...
                    active_team_ids: vec!["iron-sigil".into(), "ember-veil".into()],
        ...
        assert_eq!(
            client
                .resolved_remote_tournament_control_plane()
                .and_then(|summary| summary.standings.first())
                .map(|standing| (standing.team_id.as_str(), standing.assigned_agent_count)),
            Some(("iron-sigil", 1))
        );
        assert_eq!(
            client
                .resolved_remote_tournament_orchestration()
                .map(|summary| summary.phase),
            Some(pod_core::TournamentOrchestrationPhase::Active)
        );
        assert_eq!(
            client
                .resolved_remote_world_tournament_orchestration()
                .map(|summary| (summary.world_id.as_str(), summary.leading_team_ids.as_slice())),
            Some((
                "deadman-prime",
                ["iron-sigil".to_string()].as_slice(),
            ))
        );
        assert_eq!(
            client
