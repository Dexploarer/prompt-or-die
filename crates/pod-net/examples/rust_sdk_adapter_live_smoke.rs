...
use pod_net::{run_rust_sdk_adapter_live_smoke, RustSdkAdapterLiveSmokeConfig};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let run = run_rust_sdk_adapter_live_smoke(&RustSdkAdapterLiveSmokeConfig {
    ...
    if options.fail_on_checks && !run.report.passed() {
        return Err("Rust SDK adapter live smoke checks failed".into());
    }
...
mod tests {
    ...
    fn parse_args_accepts_live_smoke_flags() {
        let options = parse_args_from(vec![
            "rust_sdk_adapter_live_smoke".into(),
            "--host".into(),
