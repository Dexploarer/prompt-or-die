...
**Last Checkpoint**: 2026-03-19 (Rust SDK package-native surface pass)
...
**Next Action**: Wrap the remaining benchmark/report-facing `pod-sdk` exports in package-native types so the packaged SDK surface stops leaking adapter-named reporting structs, then decide whether the lower-level `pod-net` compatibility examples should stay documented as permanent debug seams or move behind a softer deprecation note.
