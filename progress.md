...
- Extended `scripts/publish_moat_snapshots.ts` so shard-target monthly snapshots can merge a moat report with separately generated browser render-route and live topology-feed artifacts, and added matching Bun coverage in `scripts/publish_moat_snapshots.test.ts`.
- Captured the first live shard-target topology artifact at `artifacts/topology-feed-live-shard-local.json` and published the first committed monthly shard-target snapshot at `docs/benchmark-snapshots/2026-03-shard-target.json`.
...
- Closed Iteration 196: extended the moat/snapshot TypeScript layer so it now preserves `headlessTopology.tournamentOrchestration`, `topologyParity.tournament_control_plane_match`, `topologyParity.tournament_orchestration_match`, and topology-feed orchestration parity flags, then republished the 2026-03 shard-target snapshot through `scripts/run_shard_target_snapshot.ts` so historical comparisons now surface tournament/swarm orchestration drift instead of dropping it.
- Closed Iteration 197: turned tournament/swarm orchestration history into explicit regression envelopes in `scripts/compare_moat_snapshots.ts`, widened the headless topology parity contracts in the TypeScript moat/snapshot layer to include world admissions and world control planes, and revalidated the republished 2026-03 shard-target snapshot with a zero-regression self-compare.
- Closed Iteration 198: wired `scripts/compare_moat_snapshots.ts` into `scripts/run_shard_target_snapshot.ts`, added explicit `--compare-baseline` support plus comparison status/report output, and revalidated the one-command shard-target workflow against the current 2026-03 baseline with regressions treated as a hard failure.
- Closed Iteration 199: taught `scripts/run_shard_target_snapshot.ts` to auto-select the latest prior monthly shard-target snapshot as the comparison baseline, fixed the baseline-copy bug so selected baselines are preserved before publish, and revalidated the live wrapper without needing `--compare-baseline`.
- Closed Iteration 200: taught `scripts/run_shard_target_snapshot.ts` to retain the published month-over-month comparison report as `docs/benchmark-snapshots/YYYY-MM-shard-target-comparison.json`, normalized its retained baseline/candidate paths back to the published snapshot history, updated the run summary to point at that retained artifact, and revalidated the current 2026-03 shard-target workflow.
- Closed Iteration 201: added `scripts/index_benchmark_snapshots.ts` plus committed `docs/benchmark-snapshots/index.json` and `docs/benchmark-snapshots/README.md`, then wired `scripts/run_shard_target_snapshot.ts` to refresh that retained history surface automatically as part of the one-command monthly workflow.
