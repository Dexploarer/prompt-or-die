...
- `crates/pod-host/src/lib.rs` now composes and supervises multiple authority hosts cleanly, exposes aggregate live transport plus gameplay-incident control-plane snapshots, supports coordinated drain/shutdown commands, retains recent shard/supervisor ops history both in-memory and in optional per-shard JSONL archives, provides typed shard/supervisor archive-query handles above those files, exposes a process-external archive query service plus both raw relay and HTTP/SSE retained-plus-live ops facades, includes shared replay/cursor semantics above those HTTP/SSE surfaces, and now emits durable replay bookmark tokens so reconnecting consumers can resume without reconstructing raw cursor maps. The next missing piece is filtered replay or shard-selection state above those bookmarks so browser/editor consumers can stay scoped to the shard subsets they are authorized to inspect.
