...
- `crates/pod-host/src/lib.rs` now composes and supervises multiple authority hosts cleanly and exposes aggregate live transport control-plane snapshots, but it still does not carry shared incident rollups or coordinated lifecycle commands at MMO scale.
