...
- `crates/pod-host/src/lib.rs` now composes and supervises multiple authority hosts cleanly, exposes aggregate live transport plus gameplay-incident control-plane snapshots, supports coordinated drain/shutdown commands, retains recent shard/supervisor ops history both in-memory and in optional per-shard JSONL archives, provides typed shard/supervisor archive-query handles above those files, exposes a process-external archive query service plus both raw relay and HTTP/SSE retained-plus-live ops facades, and now includes shared replay/cursor semantics above those HTTP/SSE surfaces. The next missing piece is durable bookmark tokens or filtered replay state so reconnecting browser/editor consumers do not have to rebuild per-shard cursor maps manually.
