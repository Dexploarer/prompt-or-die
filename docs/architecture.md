...
- `pod-net` now exports the transport-side authority adapter from
  ...
  through `DirectConnectTransportConfig`, `TransportPolicy`, and
  `parse_bind_target(...)`, composing direct-connect server config without
...
- `crates/pod-host/src/lib.rs` now composes and supervises multiple authority hosts cleanly, exposes aggregate live transport plus gameplay-incident control-plane snapshots, supports coordinated drain/shutdown commands, retains recent shard/supervisor ops history both in-memory and in optional per-shard JSONL archives, provides typed shard/supervisor archive-query handles above those files, exposes a process-external archive query service plus both raw relay and HTTP/SSE retained-plus-live ops facades, includes shared replay/cursor semantics above those HTTP/SSE surfaces, emits durable replay bookmark tokens so reconnecting consumers can resume without reconstructing raw cursor maps, supports supervisor shard-selection replay state above those bookmarks, and now enforces shard-scoped HTTP authorization through a shared `OpsHttpAuthorizationPolicySource` that can load local JSON policy documents at request time. The next missing piece is signed or process-external authz policy distribution above those local files so browser/editor consumers do not depend on local JSON policy rollout.
